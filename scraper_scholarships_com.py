...
def extract_scholarship_data(url):
    """
    Phase 2: Extract details with Smart Text Matching for location/state
    """
    try:
        ...
        # 3. Extract multiple text sections
        description = "N/A"
        details = "N/A"
        eligibility = "N/A"
        ...
        # ---------------------------------------------------------
        # 4. Smart Location Search (Independent of class names)
        # ---------------------------------------------------------
        location = "USA"
        contact_h2 = soup.find('h2', string=lambda text: text and 'Contact' in text)
        
        if contact_h2:
            contact_parent = contact_h2.parent
            if contact_parent:
                # Extract all strings in the contact section as a list
                lines = [line for line in contact_parent.stripped_strings]
                
                # Search for a line containing both a (comma) and (digits) - typical for "City, ST Zip"
                for line in lines:
                    if ',' in line and any(char.isdigit() for char in line):
                        location = f"{line} (USA)"
                        break
                
                # Fallback: if no comma found, use the 5th line as a precaution
                if location == "USA" and len(lines) >= 5:
                    location = f"{lines[4]} (USA)"
        # ---------------------------------------------------------
        
        # 5. Build enhanced data structure for JSON
        data = {
